...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
		{
			throw new TreeException("Index can't be below 0");
		}
	...
	//	?? Call childrenString.split(). Check the API for details. The result will be an array
		...
		else
		{
			...
			if (parentNode == null)
			{
				throw new TreeException("ParentNode is not found");
			}
		...
		for (String childNode : childrenArray)
		{
			TreeNode newNode = new TreeNode(childNode);
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("node 1 not exist");
		TreeNode node2 = root.getNodeWithName(name2);	// node whose name is name2
		if (node2 == null)
		{
			throw new TreeException("node 2 not exist");
		}
